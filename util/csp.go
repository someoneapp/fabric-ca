...
func ConfigureBCCSP(optsPtr **factory.FactoryOpts, mspDir, homeDir string) error {
	...
	if strings.ToUpper(opts.ProviderName) == "SW" {
		...
		/*
			if opts.SwOpts.PubAlgo == "" {
				opts.SwOpts.PubAlgo = "ecdsa"
			}
			if opts.SwOpts.KeySize == 0 {
				opts.SwOpts.KeySize = 256
			}
		*/
	...
	/*
		if opts.ProviderName == "SansecPKCS11" {
			if opts.SansecP11Opts == nil {
				opts.SansecP11Opts = &sansec.SansecP11Opts{}
			}
			if opts.SansecP11Opts.HashFamily == "" {
				opts.SansecP11Opts.HashFamily = "SHA2"
			}
			if opts.SansecP11Opts.SecLevel == 0 {
				opts.SansecP11Opts.SecLevel = 256
			}
			if opts.SansecP11Opts.PubAlgo == "" {
				opts.SansecP11Opts.PubAlgo = "SM2"
			}
			if opts.SansecP11Opts.KeySize == 0 {
				opts.SansecP11Opts.KeySize = 256
			}
			if opts.SansecP11Opts.Library == "" {
				opts.SansecP11Opts.Library = "/usr/lib/libupkcs11.so"
			}
			if opts.SansecP11Opts.Pin == "" {
				opts.SansecP11Opts.Pin = "66666666"
			}
			if opts.SansecP11Opts.Label == "" {
				opts.SansecP11Opts.Label = "Sansec HSM"
			}
			if opts.SansecP11Opts.FileKeystore == nil {
				opts.SansecP11Opts.FileKeystore = &sansec.FileKeystoreOpts{}
			}
			// The mspDir overrides the KeyStorePath; otherwise, if not set, set default
			if mspDir != "" {
				opts.SansecP11Opts.FileKeystore.KeyStorePath = path.Join(mspDir, "keystore")
			} else if opts.SwOpts.FileKeystore.KeyStorePath == "" {
				opts.SansecP11Opts.FileKeystore.KeyStorePath = path.Join("msp", "keystore")
			}
		}
	*/
	...
	//	if opts.SansecP11Opts != nil {
	//		log.Debugf("Initializing BCCSP with SansecP11 options %+v", opts.SansecP11Opts)
	//	}
...
func getBCCSPKeyOpts(kr csr.KeyRequest, ephemeral bool) (opts bccsp.KeyGenOpts, err error) {
	...
	case "SM2":
		if kr.Size() != 256 {
			return nil, fmt.Errorf("Invalid SM2 key size: %d, only support 256", kr.Size())
		}
...
// BCCSP Signer
func LoadX509KeyPair(certFile, keyFile string, csp bccsp.BCCSP) (*csptls.Certificate, error) {
	...
	if keyFile != "" {
		...
		fallbackCerts, err := csptls.LoadX509KeyPair(certFile, keyFile)
		if err != nil {
...
/*
func GetAlgoAndSize(opts *factory.FactoryOpts) (string, int, error) {
	if opts.ProviderName == "SW" {
		return opts.SwOpts.PubAlgo, opts.SwOpts.KeySize, nil
	} else if opts.ProviderName == "PKCS11" {
		return opts.Pkcs11Opts.PubAlgo, opts.Pkcs11Opts.KeySize, nil
	} else if opts.ProviderName == "SansecPKCS11" {
		return opts.SansecP11Opts.PubAlgo, opts.SansecP11Opts.KeySize, nil
	}
	return "", 0, errors.New("failed get algo and size")
