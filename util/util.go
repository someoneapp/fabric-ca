...
func CreateToken(csp bccsp.BCCSP, cert []byte, key bccsp.Key, body []byte) (string, error) {
	...
	*/
	...
	case *sm2.PublicKey:
		token, err = GenSM2Token(csp, cert, key, body)
		if err != nil {
			return "", err
		}
	}
...
func GenSM2Token(csp bccsp.BCCSP, cert []byte, key bccsp.Key, body []byte) (string, error) {
	b64body := B64Encode(body)
	b64cert := B64Encode(cert)
	bodyAndcert := b64body + "." + b64cert
...
func GetX509CertificateFromPEM(cert []byte) (*x509.Certificate, error) {
	...
	x509Cert, err := cspx509.ParseCertificate(block.Bytes)
	if err != nil {
...
func GenerateTcertPubK(pubK interface{}, mac hash.Hash) (interface{}, error) {
	switch pubK.(type) {
	case *sm2.PublicKey:
		pub := pubK.(*sm2.PublicKey)
	...
	case *ecdsa.PublicKey:
		...
		return txPub, nil
	default:
		return nil, errors.New("NOT SUPPROTED PUBLICKEY ALGORITHM")
	}
	return nil, errors.New("NOT SUPPROTED PUBLICKEY ALGORITHM")
}
