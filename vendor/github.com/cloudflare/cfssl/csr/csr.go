...
type BCCSPKeyRequest struct {
	A string `json:"algo" yaml:"algo"`
	S int    `json:"size" yaml:"size"`
}

func NewBCCSPKeyRequest(algo string, keySize int) *BCCSPKeyRequest {
	return &BCCSPKeyRequest{algo, keySize}
}

func (kr *BCCSPKeyRequest) Algo() string {
	return kr.A
}

// Size returns the requested key size.
func (kr *BCCSPKeyRequest) Size() int {
	return kr.S
}

func (kr *BCCSPKeyRequest) Generate() (crypto.PrivateKey, error) {
	return nil, nil
}

func (kr *BCCSPKeyRequest) SigAlgo() x509.SignatureAlgorithm {
	return 0
}
...
func Generate(priv crypto.Signer, req *CertificateRequest) (csr []byte, err error) {
	sigAlgo := cspx509.SignerAlgo(priv)
	if sigAlgo == cspx509.UnknownSignatureAlgorithm {
	...
	csr, err = cspx509.CreateCertificateRequest(rand.Reader, &tpl, priv)
	if err != nil {
		log.Errorf("failed to generate a CSR: %v", err)
		err = cferr.Wrap(cferr.CSRError, cferr.BadRequest, err)
		return
