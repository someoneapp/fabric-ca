...
func ParseCertificateRequest(s Signer, csrBytes []byte) (template *x509.Certificate, err error) {
	csrv, err := cspx509.ParseCertificateRequest(csrBytes)
	if err != nil {
		//csrv, err = cspx509.ParseCertificateRequest(csrBytes)
		//if err != nil {
	...
	err = cspx509.CheckSignature(csrv, csrv.SignatureAlgorithm, csrv.RawTBSCertificateRequest, csrv.Signature)
	if err != nil {
...
func ComputeSKI(template *x509.Certificate) ([]byte, error) {
	...
	encodedPub, err := cspx509.MarshalPKIXPublicKey(pub)
	if err != nil {
		return nil, err
	}
