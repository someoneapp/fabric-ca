...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	if len(c.peerCertificates) > 0 {
		...
		if certVerify.hasSignatureAndHash {
			...
			case *sm2.PublicKey:
				signatureAndHash.signature = signatureSM2
			}
		}

		switch key := pub.(type) {
		case *sm2.PublicKey:
			if signatureAndHash.signature != signatureSM2 {
				err = errors.New("tls: bad signature type for client's SM2 certificate")
				break
			}
			sm2Sig := new(sm2Signature)
			if _, err = asn1.Unmarshal(certVerify.signature, sm2Sig); err != nil {
				break
			}
			if sm2Sig.R.Sign() <= 0 || sm2Sig.S.Sign() <= 0 {
				err = errors.New("tls: SM2 signature contained zero or negative values")
				break
			}
			var digest []byte
			...
			if !sm2.Verify(key, digest, sm2Sig.R, sm2Sig.S) {
...
func (hs *serverHandshakeState) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	for i, asn1Data := range certificates {
		if certs[i], err = cspx509.ParseCertificate(asn1Data); err != nil {
			c.sendAlert(alertBadCertificate)
	...
	if c.config.ClientAuth >= VerifyClientCertIfGiven && len(certs) > 0 {
		...
		chains, err := cspx509.Verify(certs[0], opts)
		if err != nil {
			c.sendAlert(alertBadCertificate)
	...
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			if candidate.flags&suiteECDHE != 0 {
				...
				if candidate.flags&suiteSM2 != 0 {
					if !hs.sm2ok {
						continue
					}
				} else if candidate.flags&suiteECDSA != 0 {
					if !hs.ecdsaOk {
						continue
					}
