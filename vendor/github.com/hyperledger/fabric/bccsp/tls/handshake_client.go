...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		for i, asn1Data := range certMsg.certificates {
			cert, err := cspx509.ParseCertificate(asn1Data)
			if err != nil {
		...
		if !c.config.InsecureSkipVerify {
			opts := cspx509.VerifyOptions{
				...
				Intermediates: cspx509.NewCertPool(),
			}
			...
			c.verifiedChains, err = cspx509.Verify(certs[0], opts)
			if err != nil {
				c.sendAlert(alertBadCertificate)
				return err
			}
...
var tls11SignatureSchemes = []SignatureScheme{ECDSAWithP256AndSHA256, ECDSAWithP384AndSHA384, ECDSAWithP521AndSHA512, SM2WithSM3, PKCS1WithSHA256, PKCS1WithSHA384, PKCS1WithSHA512, PKCS1WithSHA1}
...
findCert:
	for i, chain := range c.config.Certificates {
		if !rsaAvail && !ecdsaAvail && !sm2Avail {
			continue
		}
		...
		for j, cert := range chain.Certificate {
			...
			if j != 0 || x509Cert == nil {
				var err error
				if x509Cert, err = cspx509.ParseCertificate(cert); err != nil {
					c.sendAlert(alertInternalError)
			...
			case sm2Avail && x509Cert.PublicKeyAlgorithm == x509.ECDSA:
			default:
				continue findCert
			}
