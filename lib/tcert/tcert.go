...
func (tm *Mgr) GetBatch(req *GetBatchRequest, ecert *x509.Certificate) (*GetBatchResponse, error) {
	...
	//pub := ecert.PublicKey.(*sm2.PublicKey)
	mac := hmac.New(sha512.New384, []byte(createHMACKey()))
	raw, _ := cspx509.MarshalPKIXPublicKey(ecert.PublicKey)
	mac.Write(raw)
	kdfKey := mac.Sum(nil)
	...
	for i := 0; i < numTCertsInBatch; i++ {
		...
		mac.Write(tidx)
		var txPub = ecert.PublicKey

		txPub, err := util.GenerateTcertPubK(ecert.PublicKey, mac)
		if err != nil {
			return nil, err
		}
		...
		raw, err := cspx509.CreateCertificate(rand.Reader, template, tm.CACert, txPub, tm.CAKey)
		if err != nil {
