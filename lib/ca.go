...
type CA struct {
	...
	verifyOptions *cspx509.VerifyOptions
...
func (ca *CA) getCACert() (cert []byte, err error) {
	...
	if ca.Config.Intermediate.ParentServer.URL != "" {
		...
		if ca.Config.CSR.KeyRequest == nil {
			req.KeyRequest = cfcsr.NewBCCSPKeyRequest("SM2", 256)
		} else {
			req.KeyRequest = cfcsr.NewBCCSPKeyRequest(ca.Config.CSR.KeyRequest.Algo(), ca.Config.CSR.KeyRequest.Size())
		}
...
func (ca *CA) VerifyCertificate(cert *x509.Certificate) error {
	...
	_, err = cspx509.Verify(cert, *opts)
	if err != nil {
...
func (ca *CA) getVerifyOptions() (*cspx509.VerifyOptions, error) {
	...
	rootCert, err := cspx509.ParseCertificate(block.Bytes)
	if err != nil {
		return nil, fmt.Errorf("Failed to parse root certificate: %s", err)
	}
	rootPool := cspx509.NewCertPool()
	rootPool.AddCert(rootCert)
	var intPool *cspx509.CertPool
	if len(rest) > 0 {
		intPool = cspx509.NewCertPool()
		if !intPool.AppendCertsFromPEM(rest) {
			return nil, errors.New("Failed to add intermediate PEM certificates")
		}
	}
	ca.verifyOptions = &cspx509.VerifyOptions{
		Roots:         rootPool,
