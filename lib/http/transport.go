...
type Transport struct {
	...
	// automatically.
	TLSNextProto map[string]func(authority string, c *csptls.Conn) http.RoundTripper
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (*persistConn, error) {
	...
	if tlsDial {
		...
		if tc, ok := pconn.conn.(*csptls.Conn); ok {
	...
	if cm.targetScheme == "https" && !tlsDial {
		...
		tlsConn := csptls.Client(plainConn, cfg)
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocolIsMutual && s.NegotiatedProtocol != "" {
		if next, ok := t.TLSNextProto[s.NegotiatedProtocol]; ok {
			return &persistConn{alt: next(cm.targetAddr, pconn.conn.(*csptls.Conn))}, nil
		}
	}
...
func (pc *persistConn) mapRoundTripErrorFromReadLoop(req *http.Request, startBytesWritten int64, err error) (out error) {
	if err == nil {
		return nil
	}
	...
	if pc.isBroken() {
		<-pc.writeLoopDone
		if pc.nwrite == startBytesWritten && outgoingLength(req) == 0 {
...
func (pc *persistConn) mapRoundTripErrorAfterClosed(req *http.Request, startBytesWritten int64) error {
	if err := pc.canceled(); err != nil {
		return err
	}
	err := pc.closed
	...
	for {
		...
		case wr := <-pc.writech:
			...
			if err != nil {
				...
				if pc.nwrite == startBytesWritten && outgoingLength(wr.req.Request) == 0 {
