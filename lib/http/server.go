...
func (w *response) WriteHeader(code int) {
	...
	if cl := get(w.handlerHeader, "Content-Length"); cl != "" {
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	te := get(header, "Transfer-Encoding")
	hasTE := te != ""
	...
	if w.handlerDone.isSet() && !trailers && !hasTE && bodyAllowedForStatus(w.status) && get(header, "Content-Length") == "" && (!isHEAD || len(p) > 0) {
	...
	if w.wants10KeepAlive && keepAlivesEnabled {
		sentLength := get(header, "Content-Length") != ""
		if sentLength && get(header, "Connection") == "keep-alive" {
	...
	if get(header, "Connection") == "close" || !keepAlivesEnabled {
		w.closeAfterReply = true
	}
	...
	if w.closeAfterReply && (!keepAlivesEnabled || !hasToken(get(cw.header, "Connection"), "close")) {
		delHeader("Connection")
...
var connStateInterface = [...]interface{}{
	http.StateNew:      http.StateNew,
	http.StateActive:   http.StateActive,
	http.StateIdle:     http.StateIdle,
	http.StateHijacked: http.StateHijacked,
	http.StateClosed:   http.StateClosed,
}
...
func (c *conn) serve(ctx context.Context) {
	...
	if tlsConn, ok := c.rwc.(*csptls.Conn); ok {
		if d := c.server.ReadTimeout; d != 0 {
...
type Server struct {
	...
	TLSConfig *csptls.Config // optional TLS config, used by ListenAndServeTLS
	...
	// automatically.
	TLSNextProto map[string]func(*Server, *csptls.Conn, http.Handler)
...
func (s *Server) closeIdleConns() bool {
	...
	for c := range s.activeConn {
		st, ok := c.curState.Load().(http.ConnState)
		if !ok || st != http.StateIdle {
			quiescent = false
			continue
		}
		c.rwc.Close()
...
func (srv *Server) Serve(l net.Listener) error {
	...
	ctx = context.WithValue(ctx, http.LocalAddrContextKey, l.Addr())
	for {
		rw, e := l.Accept()
		if e != nil {
			select {
...
func (srv *Server) ListenAndServeTLS(certFile, keyFile string) error {
	...
	if !configHasCert || certFile != "" || keyFile != "" {
		var err error
		config.Certificates = make([]csptls.Certificate, 1)
		config.Certificates[0], err = csptls.LoadX509KeyPair(certFile, keyFile)
		if err != nil {
			return err
		}
	}
	...
	tlsListener := csptls.NewListener(tcpKeepAliveListener{ln.(*net.TCPListener)}, config)
